...
public class PermissionFixController {
    ...
    public Result<String> fixPermissions() {
        ...
        List<PermissionToCreate> permissionsToAdd = Arrays.asList(
            new PermissionToCreate("首页-查看", "dashboard:view", 1L, 2, 1),
            new PermissionToCreate("待办-查看", "todo:view", 0L, 3, 1),
            new PermissionToCreate("待办-管理", "todo:manage", 0L, 3, 2),
            new PermissionToCreate("咨询-查看", "consultation:view", 0L, 3, 3),
            new PermissionToCreate("咨询-管理", "consultation:manage", 0L, 3, 4),
            new PermissionToCreate("咨询-查询", "consultation:query", 0L, 3, 5),
            new PermissionToCreate("报价单-查看", "quotation:view", 0L, 3, 6),
            new PermissionToCreate("报价单-管理", "quotation:manage", 0L, 3, 7),
            new PermissionToCreate("报价单-查询", "quotation:query", 0L, 3, 8)
        );
        ...
        for (SysPermission permission : allPermissions) {
            ...
            Integer exists = rolePermissionMapper.exists(roleId, permission.getId());
